...
function PlayerBoardLegacy({
    ...
    const countDisabledInputsForPlayerFn = () => {
        let count = 0;
        Object.entries(genreData).forEach(([genreGroup]) => {
    ...
    }, [disabledInputsCountForPlayer, setDisabledIsAll]);

    const gameEnd = () => totalMovies === totalInputs;

    if (gameEnd()) {
        navigate('/scoringLegacy', { state: { playersSelectedMovies, genreData, players, year, genreMapping, fetchedMovies } });
    }
...
export default PlayerBoardLegacy;
